...
pub mod token_wrapper {
    ...
    pub fn swap_to_wrapped(ctx: Context<Swap>, amount:u64) -> Result<()> {
        ...
        require_gte!(ctx.accounts.buyer_mint_ata.amount, amount, WrapError::InsuffientSourceTokens);
    ...
    pub fn swap_to_source(ctx: Context<Swap>, amount:u64) -> Result<()> {
        ...
        require_gte!(ctx.accounts.buyer_wrapped_mint_ata.amount, amount, WrapError::InsuffientWrapTokens);

        // Not really necessary since the swap is 1:1, so it holds as much as it has given out.
        //require_gte!(ctx.accounts.vault.amount, amount, WrapError::InsufficientVaultLiquidity);
...
#[error_code]
pub enum WrapError{
    #[msg("Insufficient vault liquidity to complete the swap.")]
    InsufficientVaultLiquidity,
    #[msg("Insufficient source tokens in the user's account to complete the swap.")]
    InsuffientSourceTokens,
    #[msg("Insufficient wrapped tokens in the user's account to complete the swap.")]
